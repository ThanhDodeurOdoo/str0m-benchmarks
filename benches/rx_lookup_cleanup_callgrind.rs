//! deterministic Callgrind benchmark for the receive lookup cleanup cadence
//!
//! the fixture builds connected RTP-mode peers, seeds the receive lookup cache
//! and measures only packet receive work inside one cleanup interval
...
fn rx_lookup_1_stream() -> RxLookupCleanupFixture {
    RxLookupCleanupFixture::new(1, configured_measured_packets())
}

fn rx_lookup_configured_streams() -> RxLookupCleanupFixture {
    RxLookupCleanupFixture::new(configured_lookup_streams(), configured_measured_packets())
}
...
fn callgrind_config() -> LibraryBenchmarkConfig {
    ...
    config.pass_through_envs(["RX_LOOKUP_STREAMS", "RX_LOOKUP_PACKETS"]);
    config
}

#[library_benchmark(config = callgrind_config())]
#[bench::one_stream(setup = rx_lookup_1_stream)]
#[bench::configured_streams(setup = rx_lookup_configured_streams)]
fn receive_packets(mut fixture: RxLookupCleanupFixture) -> usize {
    black_box(fixture.receive())
}

library_benchmark_group!(
    name = rx_lookup_cleanup_callgrind;
    benchmarks = receive_packets
);

main!(library_benchmark_groups = rx_lookup_cleanup_callgrind);
